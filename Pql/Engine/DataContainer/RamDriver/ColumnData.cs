...
namespace Pql.Engine.DataContainer.RamDriver
{
    internal sealed class ColumnData<T> : ColumnDataBase
    {
        ...
        public override DbType DbType { get { return m_dbType; } }
        ...
        private Expression GenerateReadItemExpression(Type itemType, Expression dataElement, ParameterExpression reader)
        {
            ...
            {
                var method = reader.Type.GetMethod("ReadInt64");
                var timespanctr = typeof(TimeSpan).GetConstructor(new[] { typeof(Int64) });
            ...
            {
                return Expression.Assign(dataElement, Expression.Call(this.GetType(), "ReadByteArray", null, reader));
            }

            if (itemType.IsGuid())
            {
                return Expression.Assign(dataElement, Expression.Call(this.GetType(), "ReadGuid", null, reader));
            }
        ...
        private Expression GenerateWriteItemExpression(Type itemType, Expression dataElement, ParameterExpression writer)
        {
            ...
            if (itemType.IsDateTime())
            {
                var method = writer.Type.GetMethod("Write", new[] { typeof(Int64) });
            ...
            if (itemType.IsBinary())
            {
                ...
                return Expression.Block(
                    Expression.Call(this.GetType(), "Write7BitEncodedInt", null, writer, length),
                    Expression.IfThen(
