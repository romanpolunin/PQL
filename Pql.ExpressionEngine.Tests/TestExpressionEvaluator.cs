...
namespace Pql.ExpressionEngine.UnitTest
{
    ...
    public class TestExpressionEvaluator
    {
        ...
        public void TestDebug()
        {
            ...
            var processor = _runtime.Compile<float, bool>("case case (@context) WHEN -1 then true WHEN (cast(1+0.5,'Single')) THEN true END WHEN true THEN false ELSE true end");
        ...
        public void TestDateTimesAndTimeSpans()
        {
            ...
            var eval = _runtime.Compile<DateTime, bool>("@context <= convert('12-13-2013', 'DateTime')");
            ...
            eval = _runtime.Compile<DateTime, bool>("@context <= convert('13-Dec-2013', 'DateTime')");
            Assert.IsTrue(eval(december132013));
            Assert.IsTrue(eval(december132013.AddDays(-1)));
            Assert.IsFalse(eval(december132013.AddHours(1)));

            var eval2 = _runtime.Compile<DateTime, TimeSpan>("@context - convert('13-Dec-2013', 'DateTime')");
            Assert.AreEqual(addOneDay, eval2(december132013.AddDays(1)));

            var eval3 = _runtime.Compile<DateTime, DateTime>("@context + convert('01:00:00', 'TimeSpan')");
            Assert.AreEqual(december132013 + addOneHour, eval3(december132013));

            eval3 = _runtime.Compile<DateTime, DateTime>("convert('01:00:00', 'TimeSpan') + @context");
            Assert.AreEqual(december132013 + addOneHour, eval3(december132013));

            var eval4 = _runtime.Compile<TimeSpan, TimeSpan>("@context + convert('01:00:00', 'TimeSpan')");
            Assert.AreEqual(addOneHour + addOneHour, eval4(addOneHour));

            var eval5 = _runtime.Compile<object?, DateTime>("ToDateTime(2001,10,1,12,13,14)");
        ...
        {
            var evalInt = _runtime.Compile<int, bool>("@context between 1 and 5");
            ...
            evalInt = _runtime.Compile<int, bool>("@context not between 1 and 5");
            Assert.IsTrue(evalInt(-1));
            ...
            evalInt = _runtime.Compile<int, bool>("@context between 5 and 1");
            Assert.IsFalse(evalInt(0));
            ...
            var evalDouble = _runtime.Compile<Double, bool>("@context + 1 not between 1.0 and 5.0");
            ...
            var evalString = _runtime.Compile<string, bool>("@context between 'abc' and 'ABH'");
            ...
            var evalDate = _runtime.Compile<DateTime, bool>("@context between convert('Dec-1-2000', 'DateTime') and convert('Dec-1-2001', 'DateTime')");
            ...
            var evalTimeSpan = _runtime.Compile<TimeSpan, bool>("@context between convert('00:00:05', 'TimeSpan') and convert('01:59:06', 'TimeSpan')");
        ...
        public void TestCaseStatement()
        {
            ...
            var eval3 = _runtime.Compile<bool, bool>("case @context when false then true else false end");
        ...
        public void TestAutoFieldAndPropertyDiscovery()
        {
            ...
            var simpleContextEval = _runtime.Compile<SomeItem, bool>("EtlAccountId = 1");
            data.EtlAccountId = 1;
            Assert.IsTrue(simpleContextEval(data));
            data.EtlAccountId = 0;
            Assert.IsFalse(simpleContextEval(data));

            var nestedEval = _runtime.Compile<SomeItem, long>("@Context.EtlAccountId");
            data.EtlAccountId = 1;
            Assert.AreEqual(data.EtlAccountId, nestedEval(data));
            data.EtlAccountId = 2;
            Assert.AreEqual(data.EtlAccountId, nestedEval(data));

            var nestedEval2 = _runtime.Compile<SomeItem, int>("@Context.SomeContainerData.Length");
            ...
            nestedEval2 = _runtime.Compile<SomeItem, int>("SomeContainerData.Length");
            data.SomeContainerData = new SomeContainer[0];
            Assert.AreEqual(data.SomeContainerData.Length, nestedEval2(data));
            data.SomeContainerData = new SomeContainer[10];
            Assert.AreEqual(data.SomeContainerData.Length, nestedEval2(data));
        ...
        {
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Function, "CustomEndsWith", (root, state) =>
            {
        ...
        public void TestAnalyzerExtensionsAtomHandlers()
        {
            ...
            _runtime.RegisterDynamicAtomHandler(new AtomMetadata(AtomType.Identifier, "RootLevelFields", (root, state) =>
            {
            ...
            var eval = _runtime.Compile<object?, int>("\"root.id.id\".Year");
            Assert.AreEqual(dateObject.Year, eval(null));

            eval = _runtime.Compile<object?, int>("isdefault.DayOfYear");
            Assert.AreEqual(defaultDateObject.DayOfYear, eval(null));

            var eval2 = _runtime.Compile<object?, DateTime>("isdefault");
        ...
        public void TestRuntimeExtensions()
        {
            ...
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Function, "Custom_EndsWith", customEndsWith));
            ...
            Func<SomeDataObject, int, string?> getProperty = (c, ix) => c.StringValues?[ix];
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Function, "_StringArr", getProperty));

            var data = new SomeDataObject { StringValues = new[] { "aa", "bb" } };
            var result = Eval<SomeDataObject, string>("_StringArr(@context, 0)", data);
            Assert.AreEqual(result, "aa");

            Func<string[]?, int, string?> getStringArrayItem = (c, ix) => c?[ix];
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Function, "StringAt", getStringArrayItem));

            Func<SomeDataObject, SomeDataObject> ctx = c => c;
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Identifier, "Contract", ctx));
        ...
        public void TestEvaluationCorrectness()
        {
            ...
            var nullStringAtom = new AtomMetadata(AtomType.Identifier, "nullStringValue", (Func<object, String?>)(_ => null));

            _runtime.RegisterAtom(boolAtom);
            _runtime.RegisterAtom(int16Atom);
            _runtime.RegisterAtom(int64Atom);
            _runtime.RegisterAtom(doubleAtom);
            _runtime.RegisterAtom(decimalAtom);
            _runtime.RegisterAtom(stringAtom);
            _runtime.RegisterAtom(nullStringAtom);
            ...
            Assert.IsTrue(_runtime.Compile<string?, bool>("IsDefault(@context)")(null));
            Assert.IsTrue(_runtime.Compile<string, bool>("IsDefault(@context)")(string.Empty));
            Assert.IsTrue(_runtime.Compile<string, bool>("IsDefault(@context)")(""));
        ...
        {
            Assert.AreEqual(1.5 * 2.6, ((Func<Double>)_runtime.Compile("1.5 * 2.6", typeof(Double)))());
            Assert.AreEqual(-1, ((Func<int>)_runtime.Compile("1 + 2*(3-4)", typeof(Int32)))());
            Assert.AreEqual(Double.NegativeInfinity, ((Func<Double>)_runtime.Compile("-1.0/0", typeof(Double)))());
            Assert.IsTrue(((Func<bool>)_runtime.Compile("1 > -1.5", typeof(bool)))());
            Assert.IsTrue(((Func<bool>)_runtime.Compile("'xyz' > 'abc'", typeof(bool)))());
            Assert.IsTrue(((Func<bool>)_runtime.Compile("false OR true", typeof(bool)))());
            Assert.IsTrue(((Func<bool>)_runtime.Compile("false XOR true", typeof(bool)))());
        ...
        public void TestContract()
        {
            ...
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Identifier, "Contract", stateless));
            _runtime.RegisterAtom(new AtomMetadata(AtomType.Identifier, "ContractCaptured", stateful));

            // where Contract.Id = 1
            Assert.IsTrue((_runtime.Compile<SomeDataObject, bool>("ID = 1"))(data));
            Assert.IsTrue((_runtime.Compile<SomeDataObject, bool>("Contract.ID = 1"))(data));
            ...
            Assert.IsTrue((_runtime.Compile<SomeDataObject, bool>("Name = 'test'"))(data));
            ...
            Assert.IsFalse((_runtime.Compile<SomeDataObject, bool>("Contains(Number, 'x') AND status = 2"))(data));
        }
