Pocket Query Laundromat
================

# Overview

### C#.NET dynamic expressions parser and compiler, using SQL-like expression syntax
- https://github.com/romanpolunin/PQL/tree/master/Pql/ExpressionEngine
- https://github.com/romanpolunin/PQL/tree/master/Pql/ExpressionEngine.UnitTest

This thing can still be useful in 2020 :)

TODO: create a NuGet package

### C#.NET in-memory SQL engine with fully featured WCF-based ADO.NET client driver 

Lightweight SQL, no JOIN, GROUP BY etc. but does have ORDER BY and WHERE, INSERT/DELETE/UPDATE, and some pretty extensive support for parameterization on IDbCommand

This is just a fairly extensive exercise in server-side concurrency, memory management, and streaming over network

Uses a native allocator, and a collection class from Intel's TBB (Thread Building Blocks) library - TBB used to outperform .NET 4x ConcurrentDictionary by far when dealing with 5+ millions of objects

**!LICENSE WARNING! Intel TBB used here is only free for GPL-licensed projects, so proceed with caution if you want to reuse the code which depends on it - may need to acquire a commercial license for your case.**
